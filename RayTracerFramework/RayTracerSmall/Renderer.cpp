...
Vec3f Renderer::trace(const Vec3f & rayorig, const Vec3f & raydir,const Scene & scene, const int & depth)
{
	//if (raydir.length() != 1) std::cerr << "Error " << raydir << std::endl;
	float tnear = INFINITY;
	const Sphere* sphere = NULL;
...
void Renderer::render(const Scene & scene, int iteration, const char * folderName)
{
	// quick and dirty
#ifdef _DEBUG
	unsigned width = 640, height = 480;
